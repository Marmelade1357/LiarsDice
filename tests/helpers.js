...
// Simpler "Autopilot" für einen Test-Client: bietet das kleinste mögliche Gebot oder ruft
// "Lügner!", wenn nicht mehr erhöht werden kann. Die Bots handeln serverseitig selbst.
function attachAutopilot(socket, getMyId, initialState) {
  ...
  function maybeAct() {
    ...
    clearTimeout(timer);
    timer = setTimeout(() => {
      if (state.bid && (!state.canRaise || Math.random() < 0.4)) socket.emit('callLiar');
      else socket.emit('bid', { qty: state.minRaise.qty, face: state.minRaise.face });
    }, (state.rollMs || 0) + 20);
  }
  socket.on('gameState', (state) => { lastState = state; maybeAct(); });
  setTimeout(maybeAct, 0);
}
